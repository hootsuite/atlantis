...
func TestExecuteCommand_PullErr(t *testing.T) {
	t.Log("if getting the pull request fails an error should be logged")
	setup(t)
...
func TestExecuteCommand_PullParsErr(t *testing.T) {
	t.Log("if parsing the returned pull request data fails nothing should continue")
	setup(t)
	...
	ch.ExecuteCommand(fixtures.Repo, fixtures.User, fixtures.Pull.Num, nil)
